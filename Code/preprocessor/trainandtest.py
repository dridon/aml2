...
class RandomForest():
    ...
    def train(self, samples, labels):
        ...
        self.random_forest.fit(samples, labels)
...
if labelled: 
  ...
  scounter = sc.SampleCounter(words, categories, output_file, buff=len(filtered_words))
else: 
  for item in fdr: 
    filtered_words.append(item)
  scounter = sc.SampleCounter(words, categories, output_file, labelled=False, buff=len(filtered_words))
...
f = RandomForest()
f.train(samples, labels)

fdf = "../../Datasets/processed/test/filtered_words_test_stemmed.csv"
fdr = csv.reader(open(fdf, "r"))
...
test_input = scounter.process_list
x = f.predict(test_input)

print "Writing to file ..."
myfile = open("output_pi.csv", "w+")
pwriter = csv.writer(myfile,quotechar='"', quoting = csv.QUOTE_ALL)
pwriter.writerow(("id", "category"))
i = 0 
cname  = { 0: "cs", 1: "stat", 2: "physics", 3: "math" } 

for i in range(len(x)): 
  pwriter.writerow((i, cname[x[i]]))
  myfile.flush()

# pwriter.writerows([ [i, cname[x[i]] ] for i in range(len(x))])
